...
export default function HoerenPruefung() {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (testStarted && !state.isComplete) {
        e.preventDefault();
        e.returnValue = 'Test wird abgebrochen. Möchten Sie wirklich die Seite verlassen?';
  ...
  const currentPart = testData?.parts.find(part => part.teil === currentItem?.teil);
  ...
  const handleBackClick = () => {
    if (testStarted && !state.isComplete) {
      const confirmed = window.confirm('Möchten Sie wirklich abbrechen? Ihr Fortschritt geht verloren.');
      if (confirmed) {
        triggerHaptic();
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-50 py-8 px-4">
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg p-4 border border-purple-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Hören • {currentItem.teil}. Teil</p>
            </div>
            
            <div className="flex items-center gap-4">
              <div className="text-right">
                <div className="flex items-center gap-2">
                  <Clock className={`w-5 h-5 ${state.timeRemaining < 300 ? 'text-red-600' : 'text-purple-600'}`} />
                  <div className="flex flex-col">
                    <span className={`text-xl font-bold ${state.timeRemaining < 300 ? 'text-red-600' : 'text-gray-900'}`}>
      ...
      <HoerenPlayer
        ...
        onAnswerPaired={currentItem.pairedItem ? (answer) => actions.submitAnswer(currentItem.pairedItem.no, answer) : null}
        onNext={actions.nextItem}
        selectedAnswer={state.answers[currentItem.no]}
        selectedAnswerPaired={currentItem.pairedItem ? state.answers[currentItem.pairedItem.no] : null}
        showFeedback={false}
        allowReplay={false}
